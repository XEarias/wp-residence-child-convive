            ...
            if( is_page_template('advanced_search_results.php') ) {
                ...
                if( $prop_selection->have_posts() ){
                    ...
                    while ($prop_selection->have_posts()): $prop_selection->the_post(); 
                        ...
                        $pack_id              =   get_the_author_meta( 'package_id' , $parent_userID );
                        $priority =  get_post_meta($pack_id,'listing_position', true);                    
                        $priority = ($priority === false) ? 2 : intval($priority); 
                        $priority_listing[$priority][] = $post;
                    ...
                    foreach($priority_listing as $priority_tier){

                        foreach($priority_tier as $priority_property){

                            $post = $priority_property;
